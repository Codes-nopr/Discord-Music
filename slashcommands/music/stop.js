...
module.exports = {
    ...
    run: async(client, interaction) => {
        ...
        if (autoplay) await player.set('autoplay', false);
        await player.stop();
        await player.destroy();
        return interaction.followUp({embeds: [new MessageEmbed().setColor(client.defcolor).setDescription(`Music has been stopped, left voice channel.`)]})
    }
}