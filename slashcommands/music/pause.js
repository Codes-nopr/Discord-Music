...
module.exports = {
    ...
    run: async(client, interaction) => {
        ...
        if (player.paused) return interaction.followUp({embeds: [new MessageEmbed().setColor(client.errcolor).setDescription(`Music has already paused in this server.`)]});
        await player.pause(true);
        return interaction.followUp({embeds: [new MessageEmbed().setColor(client.defcolor).setDescription(`Paused current track.`)]});
    }
}
